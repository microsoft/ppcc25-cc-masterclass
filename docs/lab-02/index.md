...
Because we defined the responses of all our actions it's also pretty easy to combine them. For example we can use the GET/Events action to get an array of all events and then iterate through them to get all their tracks.
...
And the Custom Connector also knows that GET/Events returns an array (because we defined the response of the action!), so it automatically wraps the action in an **Apply to Each**
...
Awesome! Remember to **Update Connector** and before we go back to **Power Automate** to test our linked parameters make sure to also test your new action in the Custom Connector **Test** tab and add the result as a **default response**!
...
Because we added definitions for the response of all our actions, we can use that information in PowerFx as well!
